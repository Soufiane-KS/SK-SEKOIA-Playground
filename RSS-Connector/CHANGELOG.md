# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## 0.3.0 - 2026-04-07

### Changed

- Added intake_key and intake_server to connector configuration

## 0.3.1 - 2026-04-07

### Changed

- added a description

## 0.3.2 - 2026-04-07

### Changed

- added a category and logo

## 0.4.0 - 2026-04-07

### Changed

- Implemented Checkpointing to avoid duplicate feeds showing up as events

## 0.4.1 - 2026-04-08

### Fixed

- Fixed Checkpointing Logic after critical error

## 0.4.7 - 2026-04-08

### Changed

- Implemented in-memory checkpointing to avoid duplicate events without filesystem dependency

## 0.4.6 - 2026-04-08

### Changed

- Reverted to simple feedparser approach without checkpoint to isolate hanging issue

## 0.4.5 - 2026-04-08

### Fixed

- Add try/except around _fetch_and_push to catch and log silent exceptions

## 0.4.4 - 2026-04-08

### Fixed

- Use requests with timeout instead of feedparser.parse directly to avoid hanging

## 0.4.3 - 2026-04-08

### Added

- Add diagnostic log at start of _fetch_and_push to debug polling

## 0.4.2 - 2026-04-08

### Fixed

- Use self.data_path instead of self._data_path for CheckpointDatetime
